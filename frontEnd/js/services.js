materialAdmin
    ...
    .service('getStateBreadcrumbs', ['$state', function($state){
        this.get = function(){
            var states = [];
            var listOfStates = $state.current.name.split('.');
            for(var y in listOfStates){
                if(parseInt(y)===listOfStates.length-1){
                    states.push({name: listOfStates[y], url: $state.current.name})
                } else {
                    states.push({name: listOfStates[y], url: listOfStates[y]})    
                }
            }
            return listOfStates;
        }
